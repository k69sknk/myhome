...
`MABARAK_DATA_DIR` est indispensable en local : la valeur par défaut est `/data`, qui
...
- `config.py` : réglages lus depuis les variables `MABARAK_*`, plus `APP_NAME` et
  `API_SCHEMA_VERSION`
...
- `migrate.py` : commande `mabarak-migrate`, appelée au démarrage de l'add-on

## Migrations

Les migrations vivent **dans** le paquet, sous `src/mabarak_api/alembic/`, et non à côté :
